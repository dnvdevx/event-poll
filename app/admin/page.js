...
export default function AdminPage() {
  ...
  function handleAddChoice() {
    setChoices((prev) =>
      prev.length >= MAX_CHOICES ? prev : [...prev, ""]
    );
  }

  function handleRemoveChoice(i) {
    setChoices((prev) =>
      prev.length <= MIN_CHOICES ? prev : prev.filter((_, idx) => idx !== i)
    );
  }
  ...
  async function handleCreate(e) {
    ...
    if (!text || filledChoices.length < MIN_CHOICES) {
      alert(`Please enter a question and at least ${MIN_CHOICES} choices.`);
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#2a0808] to-[#4a0e0e] text-white p-6 md:p-10">
      ...
      >
        ...
        {choices.map((c, i) => (
          <div key={i} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-black/30 border border-white/10 px-4 py-2 rounded-lg outline-none focus:ring-2 ring-[#f5d98a] text-white placeholder-gray-500"
            />
            <button
              type="button"
              ...
              title="Remove choice"
              className="px-3 rounded-lg border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:hover:bg-transparent transition"
            >
              ✕
            </button>
          </div>
        ...
        <button
          ...
          className="border border-dashed border-white/20 text-gray-300 hover:text-white hover:border-[#f5d98a] disabled:opacity-30 disabled:hover:border-white/20 py-2 rounded-lg transition text-sm"
        >
          + Add choice{choices.length >= MAX_CHOICES ? ` (max ${MAX_CHOICES})` : ""}
        </button>
        <button
          disabled={saving}
          className="bg-[#c2410c] hover:bg-[#a8380e] disabled:opacity-50 text-white py-2 rounded-lg mt-2 transition"
